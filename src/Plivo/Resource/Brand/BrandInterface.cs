...
namespace Plivo.Resource.Brand
{
    ...
    public class BrandInterface : ResourceInterface
    {
            ...
            uint? type = null, uint? status = null)
        {
			...
			return ExecuteWithExceptionUnwrap(() =>
			{
				var resources = Task.Run(async () => await ListResources<ListResponse<Brands>>("10dlc/Brand",data).ConfigureAwait(false)).Result;
           ...
           uint? type = null, uint? status = null)
        {
            ...
            var resources = await ListResources<ListResponse<Brands>>("10dlc/Brand", data);
            resources.Objects.ForEach(
                (obj) => obj.Interface = this
            );
        ...
        public Brand Create(string city, string company_name, string country, string ein, string ein_issuing_country,
            string email, string entity_type,  string phone, string postal_code,
            string registration_status, string state, string stock_exchange, string stock_symbol, string street, string vertical,
            string? website = null, string? secondary_vetting = null
            string? alt_business_id_type = null, string? first_name, string? last_name, string? alt_business_id = null)
        {
